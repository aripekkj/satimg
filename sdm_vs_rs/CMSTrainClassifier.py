...
grouped = df[['segment', 'int_class']].groupby('segment').min()
# split train, test
X_train, X_test, y_train, y_test = train_test_split(grouped, grouped['int_class'], 
                                                  test_size=0.1, random_state=42, 
                                                  stratify=grouped['int_class'])
# split validation set from train
X_train, X_val, y_train, y_val = train_test_split(X_train, X_train['int_class'], 
                                                  test_size=0.12, random_state=42, 
                                                  stratify=X_train['int_class'])
print('proportion of train', len(X_train)/len(df)*100)
print('proportion of validation', len(X_val)/len(df)*100)
print('proportion of test', len(X_test)/len(df)*100)

# select fold train and test
df_train = df[df['segment'].isin(X_train.index)]
df_val = df[df['segment'].isin(X_val.index)]
df_test = df[df['segment'].isin(X_test.index)]

# select columns
X_train = df_train[traincols].to_numpy()
y_train = le.transform(df_train['int_class'])
X_val = df_val[traincols].to_numpy()
y_val = le.transform(df_val['int_class'])
X_test = df_test[traincols].to_numpy()
y_test = le.transform(df_test['int_class'])

# select groups (ie. segments)
train_groups = df_train['segment'].to_numpy()
eval_groups = df_val['segment'].to_numpy()
