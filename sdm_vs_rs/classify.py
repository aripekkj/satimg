...
# paths for rasters
fp_dir = '/mnt/d/users/e1008409/MK/OBAMA-NEXT/sdm_vs_rs/BlackSea/tiles'
fp_pcadir = os.path.join(os.path.dirname(fp_dir),'pca')
fp_bathydir = os.path.join(os.path.dirname(fp_dir), 'bathymetry')

# model filepath
modeldir = os.path.join(os.path.dirname(fp_dir), 'model')
# list models
...
#    plt.imshow(nodatamask)
    ...
    if os.path.isdir(outdir) == False:
        os.mkdir(outdir)
    outfile = os.path.join(outdir, os.path.basename(fp_tile).split('.')[0] + '_' + os.path.basename(fp_model).split('.sav')[0] + '_proba.tif')
    # update metadata
    upmeta = meta.copy()
